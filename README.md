...
Codes for different GNN models are also presented under `Models`, and complete comparison of the created models with different metrics (i.e. mean absolute error, accuracy and F-1 score) are available in `Compare Models Predictions.ipynb`. `results` folder contains the predictions (*.csv files*) for the testing dataset used to compare the models. An intuitive comparison of the GNN models and their performances are available in this blog post.
